...
namespace Components.Controller.GroundDetection
{
    /// <summary>
    ...
    public abstract class BaseGroundDetection : MonoBehaviour
    {
        ...
        {
            // Get the center and radius of the capsule collider.
            var center = capsuleCollider.center;
            var radius = capsuleCollider.radius;

            // Calculate the height of the capsule by subtracting the radius from half of the height.
            var height = capsuleCollider.height * 0.5f - radius;

            // Calculate the positions of two spheres that are centered on the top and bottom of the capsule.
            var topSphereCenter = center + Vector3.up * height;
            var bottomSphereCenter = center - Vector3.up * height;

            // Calculate the positions of the top and bottom spheres.
            var top = position + rotation * topSphereCenter;
            var bottom = position + rotation * bottomSphereCenter;

            // Use the Physics.OverlapCapsuleNonAlloc method to detect any colliders that overlap with the capsule.
            // The method takes in the bottom and top positions of the capsule, the radius of the capsule, and an array to store the colliders.
            var colliderCount = Physics.OverlapCapsuleNonAlloc(bottom, top, radius, OverlappedColliders, _overlapMask, triggerInteraction);

            // Set the overlapCount output parameter to the number of colliders that overlap with the capsule.
            ...
            for (var i = 0; i < colliderCount; i++)
            {
                ...
                // Check if the collider is not null and is not the same as the capsule collider.
                if (overlappedCollider!= null && overlappedCollider!= capsuleCollider)
                {
                    continue;
                }

                // If the collider is the same as the capsule collider, decrement the overlapCount variable.
                // This is because the capsule collider should not be included in the array of colliders.
                if (i < --overlapCount)
                {
